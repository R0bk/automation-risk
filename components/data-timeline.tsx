...
export function DataTimeline({
  ...
  return (
    <div className="w-full select-none px-6 py-6 sm:px-10">
      <div className="mx-auto max-w-[1200px]">
        <p className="mb-4 text-[10px] font-semibold uppercase tracking-[0.28em] text-[rgba(38,37,30,0.4)]">
          Data vintage
        </p>
          ...
          {points.map((point, i) => {
            ...
            return (
              ...
              >
                {/* Dot */}
                <div
                  className={`relative flex h-5 w-5 items-center justify-center rounded-full border-2 transition-all duration-200 sm:h-4 sm:w-4 ${
                    isActive
                ...
                >
                  {isActive && (
                    <div className="h-1.5 w-1.5 rounded-full bg-white sm:h-1 sm:w-1" />
                  )}
                </div>

                {/* Label */}
                <span
                  className={`absolute left-1/2 -translate-x-1/2 whitespace-nowrap text-xs tabular-nums transition-colors duration-200 ${
                    isActive
                      ? "top-7 font-semibold text-[rgba(245,78,0,0.9)] sm:top-6"
                      : point.available
                        ? "top-7 font-medium text-[rgba(38,37,30,0.55)] group-hover:text-[rgba(38,37,30,0.8)] sm:top-6"
                        : "top-7 font-medium text-[rgba(38,37,30,0.3)] sm:top-6"
                  }`}
                >
                  {point.label}
                  {!point.available && (
                    <span className="ml-1 text-[9px] uppercase tracking-[0.1em]">
                      soon
                    </span>
                  )}
                </span>
