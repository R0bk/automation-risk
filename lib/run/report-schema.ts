...
export const deprecatedOrgRoleSchema = z.object({
  ...
  parentCluster: z.string().min(1).max(256).describe("id of parent orgNodeSchema"),
...
export const aggregationSchema = z.object({
  type: z.enum(["function", "geography", "business_unit", "seniority", "other"]),
...
export const enrichedOrgRoleSchema = z.object({
  onetCode: z.string().min(1).max(32),
  title: z.string().min(1).max(256),
  normalizedTitle: z.string().min(1).max(256).optional(),
...
export const enrichedAggregationBucketSchema = z.object({
  key: z.string().min(1).max(128),
  headcount: z.number().int().nonnegative().nullable().optional(),
  automationShare: z.number().min(0).max(1).nullable().optional(),
  augmentationShare: z.number().min(0).max(1).nullable().optional(),
...
export const enrichedAggregationSchema = z.object({
  type: z.enum([
    "function",
    "geography",
    "business_unit",
  ...
  label: z.string().min(1).max(128),
  buckets: z.array(enrichedAggregationBucketSchema).max(100),
});

export const enrichedReportSourceSchema = z.object({
  title: z.string().max(256).optional(),
  url: z.string().url(),
  snippet: z.string().max(1000).optional(),
});

export const enrichedOrgReportSchema = z.object({
  metadata: z.object({
    companyName: z.string().min(1).max(256),
    companySlug: z.string().min(1).max(128),
    summary: z.string().max(2000).optional(),
...
export type EnrichedOrgNode = z.infer<typeof enrichedOrgNodeSchema>;
export type EnrichedAggregation = z.infer<typeof enrichedAggregationSchema>;
export type EnrichedOrgReport = z.infer<typeof enrichedOrgReportSchema>;
